...
fn ntp_roundtrip(host: &str, port: u16) -> Result<NTPResult, std::io::Error> {
    ...
    let t3: DateTime<Utc> = response.rx_time().unwrap().into();
...
fn check_time() -> Result<f64, std::io::Error> {
    ...
    for &server in servers.iter() {
        ...
        match calc {
            ...
            Err(_) => {
                println!(" ? [response took to long]");
            }
        };
    }

    let mut offsets: Vec<f64> = Vec::with_capacity(servers.len());
    let mut offsets_weights: Vec<f64> = Vec::with_capacity(servers.len());
...
impl Clock {
    ...
    fn set<Tz: TimeZone>(t: DateTime<Tz>) {
        ...
        u.tv_sec = t.timestamp_subsec_micros() as suseconds_t;
...
fn main() {
    let app = App::new("clock")
        ...
        .after_help(
            "Note: Unix timestamp as parsed as whole \
        seconds since 1st January 1970 0:00:00 UTC \
        for more accuraty use another format",
        )
        .arg(
