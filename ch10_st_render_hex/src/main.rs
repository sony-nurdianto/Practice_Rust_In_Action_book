...
use crate::Opertation::{Forward, Home, Noop, TurnLeft, TurnRight};
...
fn parse(input: &str) -> Vec<Opertation> {
    let mut steps = Vec::<Opertation>::new();
...
fn convert(opeartions: &Vec<Opertation>) -> Vec<Command> {
    let mut turtle = Artist::new();

    let mut path_data = Vec::<Command>::with_capacity(opeartions.len());
